...
public class DatabaseCreation {
    public static void writeToDB(ArrayList<EmployeeObject> empArrList){
        try(Connection conn = DriverManager.getConnection("jdbc:sqlite:week5DB.db")) {
            ...
            try{
                statement.executeUpdate("CREATE TABLE \"EMPLOYEE_DATABASE\" (\"EMPLOYEE_ID\" INTEGER NOT NULL, \"PREFIX\" VARCHAR(100), \"FIRST_NAME\" VARCHAR(100),\"MIDDLE_INITIAL\" VARCHAR(100), " +
                        "\"LAST_NAME\" VARCHAR(100), \"GENDER\" VARCHAR(100) ,\"EMAIL\" VARCHAR(100) , \"DATE_OF_BIRTH\" DATE, \"DATE_OF_EMPLOYMENT\" DATE, \"SALARY\" INTEGER, PRIMARY KEY(\"EMPLOYEE_ID\"))");
                statement.close();
            }catch (Exception e){
                //e.printStackTrace();
                statement.executeUpdate("DROP TABLE \"EMPLOYEE_DATABASE\"");
                writeToDB(empArrList);
                statement.close();
            }
...
//String gender, String email, Date dateOfBirth, Date dateOfEmployment, int salary